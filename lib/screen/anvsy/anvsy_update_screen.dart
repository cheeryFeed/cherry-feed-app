...
class AnvsyUpdateScreen extends StatefulWidget {
  final Anvsy anvsy;

  const AnvsyUpdateScreen({Key? key, required this.anvsy})
      : super(key: key);

  @override
  State<AnvsyUpdateScreen> createState() => _AnvsyUpdateScreenState();
}

class _AnvsyUpdateScreenState extends State<AnvsyUpdateScreen> {
  ...
  Future<void> _initWidget() async {
    ...
    setState(() {
      ...
      id = widget.anvsy.id;
      anvsyNm = widget.anvsy.anvsyNm;
      imgId = widget.anvsy.imgId;
      status = widget.anvsy.status;
      anvsyAt = widget.anvsy.anvsyAt!;
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            CoverImg(onImageUploaded: onImageUploaded,defaultImg: widget.anvsy.imgId,isActive: true,),
            SizedBox(
              height: 20,
            ),
            TitleInput(onValueChanged: setTitle, textSize: 19,placeholder: '제목을 입력해주세요.',defaultValue: widget.anvsy.anvsyNm,),
            SizedBox(
  ...
  void onPressed() async {
    ...
    final url = ApiHost.API_HOST_DEV + '/api/v1/anvsy/${id}';
    final headers = {
    ...
    if (response.statusCode == 200) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: const Text('서버와 연결할 수 없습니다.'),
          content: const Text('자원을 찾을 수 없습니다.'),
          actions: [
            TextButton(
